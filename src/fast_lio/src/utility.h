...
public:
...
#endif
    vector<double> preBiasAccV;
    vector<double> preBiasGyroV;
    Eigen::Vector3d preBiasAcc;
    Eigen::Vector3d preBiasGyro;
    ...
    ParamServer()
    {
        ...
        nh.param<std::string>(PROJECT_NAME + "/mapFrame", mapFrame, "world");
        ...
        nh.param<vector<double>>(PROJECT_NAME + "/preBiasAcc", preBiasAccV, vector<double>());
        nh.param<vector<double>>(PROJECT_NAME + "/preBiasGyro", preBiasGyroV, vector<double>());
        preBiasAcc = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(preBiasAccV.data(), 3, 1);
        preBiasGyro = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(preBiasGyroV.data(), 3, 1);
    ...
    #else
        //? mod: 修改外参读取方式
        nh.param<vector<double>>("/mapping/extrinsic_T", t_imu_lidar_V, vector<double>());
        nh.param<vector<double>>("/mapping/extrinsic_R", R_imu_lidar_V, vector<double>());
    ...
    {
        static const double imuGravity = 9.8035;
        sensor_msgs::Imu imu_out = imu_in;
        // rotate acceleration
        Eigen::Vector3d acc(imu_in.linear_acceleration.x, imu_in.linear_acceleration.y, imu_in.linear_acceleration.z);
        acc *= imuGravity;
        // std::cout << acc.transpose() << "\n";
    #if IF_OFFICIAL
        acc = extRot * acc;
    #else
        acc = R_lidar_imu * acc;
