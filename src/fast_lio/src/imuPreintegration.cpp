...
public:
    ...
    TransformFusion()
    {
        ...
        subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("/Odometry", 5, &TransformFusion::lidarOdometryHandler, this, ros::TransportHints().tcpNoDelay());
        subImuOdometry = nh.subscribe<nav_msgs::Odometry>("/odometry/imu_incremental", 2000, &TransformFusion::imuOdometryHandler, this, ros::TransportHints().tcpNoDelay());

        //; 注意：以下是全局无漂移的结果，仅用于rviz显示，而不会用于算法的其他部分
        pubImuOdometry = nh.advertise<nav_msgs::Odometry>("/odometry/imu", 2000);
        pubImuPath = nh.advertise<nav_msgs::Path>("/lidar/imu/path", 1);
    }
    ...
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg)
    {
        ...
        Eigen::Vector3f t2 = imuOdomAffineLast.rotation() * Eigen::Vector3f(-0.25, 0., -0.05) + Eigen::Vector3f(x, y, z);

        // publish latest odometry
        nav_msgs::Odometry laserOdometry = imuOdomQueue.back();
        laserOdometry.pose.pose.position.x = t2.x();
        laserOdometry.pose.pose.position.y = t2.y();
        laserOdometry.pose.pose.position.z = t2.z();
...
public:
    ...
    IMUPreintegration()
    {
        ...
        subOdometry = nh.subscribe<nav_msgs::Odometry>("/Odometry/Incremental", 5, &IMUPreintegration::odometryHandler, this, ros::TransportHints().tcpNoDelay());

        pubImuOdometry = nh.advertise<nav_msgs::Odometry>("/odometry/imu_incremental", 2000);
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg)
    {
        ...
        if (systemInitialized == false)
        {
            ...
            // initial bias
            prevBias_ = gtsam::imuBias::ConstantBias(preBiasAcc, preBiasGyro);
        ...
        if (count++ > 100) {
            std::cout << "======================================\n";
            std::cout << "bias (acc, gyro):\n" << prevBias_.vector().matrix().transpose().head(3) << "\n"
                << prevBias_.vector().matrix().transpose().tail(3) << "\n";
            // std::cout << "cov:\n" << imuIntegratorImu_->preintMeasCov().diagonal().transpose().head(3) << "\n"
            //     << imuIntegratorImu_->preintMeasCov().diagonal().transpose().segment(3, 6) << "\n"
            //     << imuIntegratorImu_->preintMeasCov().diagonal().transpose().tail(3) << "\n";
            count = 0;
        }
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr &imu_raw)
    {
        ...
        if (count++ > 700) {
            std::cout << "-------------------------------------\n";
            std::cout << "twist.linear:\n" << currentState.velocity().matrix().transpose() << "\n";
            count = 0;
        }
    }
};
